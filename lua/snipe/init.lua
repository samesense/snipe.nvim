...
Snipe.setup = function(config)
	config = config or {}
	config.custom_tags = vim.tbl_extend("force", H.default_config.custom_tags, config.custom_tags or {})
...
H.default_config = {
	custom_tags = { ["run.sh"] = "r", ["Snakefile.py"] = "s" },
	ui = {
...
H.setup_config = function(config)
	...
	vim.validate({
		["custom_tags"] = { config.custom_tags, "table", true },
	})
...
function Snipe.default_map_tags(tags)
	-- First apply any custom tags based on filename
	for i, item in ipairs(Snipe.global_items) do
		if Snipe.config.custom_tags[item.name] then
			tags[i] = Snipe.config.custom_tags[item.name]
		end
	end

	-- Then apply any manually set tags
